...
int idx_num = 10000;
int fifo_queue[10000] = {0};    // create a array to store the current idx in this set
...
uint64_t cache_sim_t::victimize(uint64_t addr)   // anyway we have to edit this code
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  
  size_t way = fifo_queue[idx];                  // sign where can put those data 
  fifo_queue[idx] = ++fifo_queue[idx] % ways;
