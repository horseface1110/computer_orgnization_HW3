...
int lru_queue[10000][100] ={0};   //create a way to store the item use info  
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx*ways + i] & ~DIRTY))
    {
      ...
      {
        lru_queue[idx][j] += 1;                     // add the sets item timer 1
      } 
      lru_queue[idx][i] = 0;                        //set this one as 0
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  for(size_t i = 0 ; i < ways ; i++)
  {
    lru_queue[idx][i] += 1;                   // timer add one 
    if(lru_queue[idx][i] >= tmp_use_timer)
    {
      tmp_use_timer = lru_queue[idx][i];
      tmp_way = i;
    }
  }

  lru_queue[idx][tmp_way] = 0 ;   // retimer to 0
  size_t way = tmp_way;
