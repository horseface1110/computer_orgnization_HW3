...
int lfu_queue[10000][10] = {0};    // create a array to store the current idx in this set
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx*ways + i] & ~DIRTY))    // if hit
    {
      lfu_queue[idx][i] += 1;                    //add the item use times
...
{
  int tmp_way = 0;   // to recort tmp_use_rate's way
  int tmp_use_rate = 10000;
  size_t idx = (addr >> idx_shift) & (sets-1);
 
  for(size_t i = 0 ; i < ways ;i++)
  {
    if(lfu_queue[idx][i] <= tmp_use_rate)       //use to find the min use rate one
    {
      tmp_use_rate = lfu_queue[idx][i];
      tmp_way = i;
    }
  }
  size_t way = tmp_way;
